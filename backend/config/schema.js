...
export async function ensureSchema() {
  const db = getDB()
  await db.collection('majors').createIndex({ report_key: 1 }, { unique: true })
  await db.collection('articulations').createIndex({ major_id: 1 })
  await db.collection('cerritos_catalog').createIndex({ course_identifier_parent_id: 1 }, { unique: true })
